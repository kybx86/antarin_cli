...
class EnterCloud(Base):

	def send_request(self,token,access_key,projectname):
		try:
			url = "http://127.0.0.1:8000/rest-enterinstance/"
			#url = "http://webapp-test.us-west-2.elasticbeanstalk.com/rest-enterinstance/"
			connection = requests.post(url, data = {'token':token,'access_key':access_key,'projectname':projectname})
		...
		if config.has_section('user_details'):
			...
			if token != "":
				if int(env_flag) == 1 and int(instance_flag)==0:
					...
					connection = EnterCloud.send_request(self, token, access_key,projectname)
