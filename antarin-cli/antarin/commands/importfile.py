...
class ImportFile(Base):
	...
	def send_request(self,token,path,env_name,instance_id,section):
		try:
			url = "http://127.0.0.1:8000/rest-importfile/"
			#url = "http://webapp-test.us-west-2.elasticbeanstalk.com/rest-importfile/"
		...
		if config.has_section('user_details'):
			...
			if token != "":
				if int(env_flag) and int(instance_flag):
					...
					connection = ImportFile.send_request(self, token, path, env_name,instance_id,section)
